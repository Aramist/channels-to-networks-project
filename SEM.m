function [snc_subsampled,stn_subsampled,gpe_subsampled,DA2,srnd,simtime]=SEM(dur,filename0,randinit,nolat,pd,lstsn,wsg1,wgs1,sthrsnc,wstsn)
...
% Number of neurons
%STN
nSTN=64; % (nSTNxnSTN network size)
Mstn=nSTN;
Nstn=nSTN;
Pstn=Mstn*Nstn;

%SNc
nSNc=16; % (nSNcxnSNc network size)
Msnc=nSNc;
Nsnc=nSNc;
Psnc=Msnc*Nsnc;

%GPe
nGPe=64; % (nGPexnGPe network size)
Mgpe=nGPe;
Ngpe=nGPe;
Pgpe=Mgpe*Ngpe;
...
% STN-SNc connections
wstnsnc_matrix=normrnd(wstsn,0.1,16,16); % Weights are randomly assigned
...
if plot == 1
    if(rastplot_GPe_STN==1)
        ...
        for neuron=(1:Pstn)
            plot(x_axis(stn_subsampled(neuron, :)==1), neuron, '.b', 'MarkerSize', 2);
            hold on;
        end
        ...
        set(gca,'fontsize',sizz);
        for neuron=(1:Pgpe)
            plot(x_axis(gpe_subsampled(neuron, :)==1), neuron, '.b', 'MarkerSize', 2);
            hold on;
        end
    ...
    if(rastplot_SNc_STN==1)
        ...
        set(gca,'fontsize',sizz);
        for neuron=(1:Psnc)
            plot(x_axis(snc_subsampled(neuron, :)==1), neuron, '.b', 'MarkerSize', 2);
            hold on;
        end
